import"./supabaseClient-BXT4n9qe.js";import{c as h,i as v,a as w}from"./layout-Cb_Oe5x8.js";import{a as m}from"./utils-D6Z1B7Jq.js";import{c as L}from"./dataService-BFNJnUJq.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.39.0/+esm";const I=(e="")=>`${e}`.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),f=()=>h(w()),S=()=>f().toLowerCase().replace(/[^a-z0-9]+/g,"_").replace(/^_+|_+$/g,"").substring(0,60)||"company";window.XLSX={utils:{json_to_sheet:e=>e,book_new:()=>({Sheets:{},SheetNames:[]}),book_append_sheet:(e,t,a)=>{e.Sheets[a]=t,e.SheetNames.push(a)}},writeFile:(e,t)=>{const a=e.SheetNames[0],n=e.Sheets[a];if(!n||n.length===0)return;const i=Object.keys(n[0]),r=[i.join(","),...n.map(b=>i.map(p=>{const x=b[p]===null||b[p]===void 0?"":b[p];return typeof x=="string"?`"${x.replace(/"/g,'""')}"`:x}).join(","))].join(`
`),s=new Blob(["\uFEFF"+r],{type:"text/csv;charset=utf-8;"}),o=document.createElement("a"),c=URL.createObjectURL(s);o.setAttribute("href",c),o.setAttribute("download",t.replace(".xlsx",".csv")),o.style.visibility="hidden",document.body.appendChild(o),o.click(),document.body.removeChild(o)}};let g="YTD";const y=I(f()),T=`
    <div class="flex flex-col space-y-8 max-w-5xl mx-auto">
        ...
        <div class="hidden print:flex justify-between items-center border-b-2 border-gray-800 pb-4 mb-4">
            ...
            <div class="text-right">
                <p class="text-sm font-bold">Report Period: ${g}</p>
        ...
        <div class="flex flex-col md:flex-row justify-between items-end border-b border-gray-200 pb-6 gap-4 print:hidden">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">${y}</h1>
...
`,l=e=>e==null||isNaN(e)?0:e;function E(){const e=["income-table-body","ratios-table-body","bs-table-body"];let t=[];e.forEach(i=>{var s;const r=(s=document.getElementById(i))==null?void 0:s.querySelectorAll("tr");r==null||r.forEach(o=>{const c=o.querySelectorAll("td");c.length>=2&&t.push({Section:i.replace("-table-body","").toUpperCase(),Item:c[0].innerText.trim(),Value:c[1].innerText.trim()})})});const a=XLSX.utils.json_to_sheet(t),n=XLSX.utils.book_new();XLSX.utils.book_append_sheet(n,a,"Financials"),XLSX.writeFile(n,`${S()}_Financial_Report_${g}.xlsx`)}const d=(e,t,a=!1,n=!1,i="")=>{let r="border-b border-gray-100 hover:bg-gray-50 transition-colors",s="px-8 py-4 text-gray-700",o="px-8 py-4 text-right font-mono text-gray-800";return n?(r="bg-gray-50 font-bold border-t-2 border-gray-100",s="px-8 py-4 text-gray-900 font-extrabold uppercase text-xs tracking-wider",o="px-8 py-4 text-right font-bold text-gray-900 text-lg"):a&&(s="px-8 py-4 font-bold text-gray-900",o="px-8 py-4 text-right font-bold text-gray-900"),i&&(o+=` ${i}`),`
        <tr class="${r}">
            <td class="${s}">${e}</td>
            <td class="${o}">${t}</td>
